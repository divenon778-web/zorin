...
local insertedIds: { [string]: string } = {}
...
local function makeThumbnailUrl(universeId: string): string?
...
local function setProjectPickerText(text: string)
	ProjectPickerBtn.Text = text or "No project selected"
end

local function setProjectMenuOpen(open: boolean)
	dropdownOpen = open
	if open then
...
local function addEmptyOutput(text: string)
...
local function showLinkOutput(codeText: string, urlText: string, noteText: string)
	clearOutput()
...
local function setProjectSelected(projectId: string)
...
local function resolveParentTarget(parentName: string, className: string, createdInstanceMap: {[string]: Instance}): Instance
...
local function insertInstances(data: {}): {[string]: Instance}
	local createdInstanceMap: {[string]: Instance} = {}
...
local function processdeletions(data: {}, createdInstanceMap: {[string]: Instance}): number
	...
	for _, entry in ipairs(deletions) do
		local ok, err = pcall(function()
			...
			local targetName: string  = tostring(entry.name   or "")
			local parentName: string  = tostring(entry.parent or "")
...
local function insertLatestData()
	...
	local created: number = 0
	local updated: number = 0
	for _, scriptData in ipairs(latestProjectData.scripts or {}) do
		local ok, err = pcall(function()
			...
			if existing and existing:IsA("LuaSourceContainer") then
				existing.Source = code; updated += 1
			else
...
local function verifyToken(token: string): (boolean, string?)
	local ok, result = pcall(function()
		return HttpService:RequestAsync({
...
local function fetchLatestForProject(projectId: string, silent: boolean)
	if not savedToken or savedToken == "" then return end
	if not silent then
...
local function showConnected(username: string)
	...
	pollThread = task.spawn(function()
		...
		while attempts < 150 do
			...
			if pResult.StatusCode == 410 then
				setStatus("Link expired. Try again.", C.red)
				LoginOverlay.Visible = true
				break
			end
		...
		if attempts >= 150 then setStatus("Still waiting...", C.amber); LoginOverlay.Visible = true end
	end)
end
