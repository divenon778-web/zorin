...
const BYNARA_BASE  = "https://router.bynara.id/v1/chat/completions"
const BYNARA_KEY  = "sk-nry-vWalB1l2t_rMiZ1EMqhjklv4M6i_2p7drvzn11sH7CE"
...
const BAI_KEY  = "sk-dkmmkuq2uct47ouhaioexh1kt3zdgjud"

const NVIDIA_BASE = "https://integrate.api.nvidia.com/v1/chat/completions"
const NVIDIA_KEY  = "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"
...
function getProviderConfig(model) {
  if (!model || typeof model !== "string") {
    return { base: GROQ_BASE, key: GROQ_KEY, provider: "groq", model: "openai/gpt-oss-120b" }
  }
...
function createThinkingStripTransform(encoder, metaLine, onFirstChunk) {
  ...
  function processText(text) {
    ...
    while (i < text.length) {
      if (inThinking) {
        const endIdx = text.indexOf("</tool_call>", i)
        if (endIdx === -1) {
          break
        }
        inThinking = false
  ...
  return new TransformStream({
    transform(chunk, controller) {
      ...
      const raw = remainder + new TextDecoder().decode(chunk)
      ...
      for (const line of lines) {
        ...
        if (line.startsWith(dataPrefix)) {
          ...
          try {
            ...
            if (delta?.content) {
              const cleaned = processText(delta.content)
              if (!cleaned && !inThinking) continue
              delta.content = cleaned
            }
            if (delta && !delta.content && !delta.role) continue
            controller.enqueue(encoder.encode(dataPrefix + JSON.stringify(parsed) + "\n"))
          } catch {
            controller.enqueue(encoder.encode(line + "\n"))
          }
        } else {
    ...
    flush(controller) {
      if (remainder) {
        const dataPrefix = "data: "
        if (remainder.startsWith(dataPrefix)) {
          const data = remainder.slice(dataPrefix.length).trim()
          if (data !== "[DONE]") {
            try {
              const parsed = JSON.parse(data)
              const delta = parsed.choices?.[0]?.delta
              if (delta?.content) {
                delta.content = processText(delta.content)
              }
              controller.enqueue(encoder.encode(dataPrefix + JSON.stringify(parsed) + "\n"))
            } catch {
              controller.enqueue(encoder.encode(remainder + "\n"))
...
export function callHcnsecStream(system, messages, model, timeoutMs = 60000) {
  const { base, key, provider, model: actualModel } = getProviderConfig(model)
  console.log(`[callHcnsecStream] model: ${model} -> ${actualModel} (${provider}), messages: ${messages.length}`)
...
export async function runGenerateStream({
  ...
  for (const m of modelOrder) {
    try {
      const { stream, provider } = await callHcnsecStream(sys, messages, m, 30000)
      if (!stream) continue
...
async function callHcnsec(system, messages, model, timeoutMs = 60000) {
  const { base, key, provider, model: actualModel } = getProviderConfig(model)
  console.log(`[callHcnsec] model: ${model} -> ${actualModel} (${provider}), messages: ${messages.length}`)
...
async function callWithFallback(system, messages, timeoutMs = 30000, preferredModel) {
  ...
  for (const model of modelOrder) {
    ...
    const modelTimeout = isDeepSeek ? 55000 : timeoutMs
...
export function normaliseResponse(result, model, reasoning = "", fellBack = false, provider = "bynara.id") {
...
export async function runGenerate({
  ...
  try {
    const { output, reasoning, model: pickedModel, fellBack: didFallBack, provider: pickedProvider } =
      await callWithFallback(sys, messages, 10000, requestedModel)
    raw           = output
