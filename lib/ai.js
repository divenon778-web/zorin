...
const TABI_BASE = "https://tabitoken.com/v1/chat/completions"
const TABI_MODEL = "claude-opus-5-thinking"
const TABI_API_KEY = process.env.TABITOKEN_API_KEY || process.env.ORCAROUTER_API_KEY || process.env.TOKENROUTER_API_KEY || "sk-AHP3yBRE4bu5zZ1vsd3ub9g3YdduzJyHSVczphZnjNDlKFOm"
...
export const ALL_MODELS = [
  ...
  // legacy aliases — still accepted, routes to TabiToken
  "deepseek/deepseek-v4-pro-free",
  "qwen/qwen3.8-max-free",
]

const DEFAULT_MODEL  = "claude-opus-5-thinking"
const STEPS_MODEL    = "claude-opus-5-thinking"
const THINKING_MODEL = "claude-opus-5-thinking"
...
function getProviderConfig(model) {
  // qwen fallback uses tokenrouter
  if (model && model.startsWith("qwen/")) {
    const tokenRouterBase = "https://api.tokenrouter.com/v1/chat/completions"
    const tokenRouterKey = process.env.TOKENROUTER_API_KEY || process.env.TABITOKEN_API_KEY || process.env.ORCAROUTER_API_KEY || ""
    return { base: tokenRouterBase, provider: "tokenrouter", model, apiKey: tokenRouterKey }
  }
  return { base: TABI_BASE, provider: "tabitoken", model: TABI_MODEL, apiKey: TABI_API_KEY }
}
...
export function normaliseResponse(result, model, reasoning = "", fellBack = false, provider = "tabitoken") {
