...
const SEEKAI_KEY  = "sk-biIVw8mvqkAYFbmCB1lfvoL8vATyk4Ivirjfq7AnAVinmeKj"
const HCNSEC_KEY  = "sk-I5e5KLNBS5kXwabGF3y92OrUEF2Tecxkcm1Jse0SY25YE6S5"

export const SEEKAI_MODELS = [
  "gpt-5.6-luna",
]

export const HCNSEC_MODELS = [
  "DeepSeek-V4-Pro",
]
...
export const ALL_MODELS = [...SEEKAI_MODELS, ...HCNSEC_MODELS, ...NVIDIA_MODELS]

const DEFAULT_MODEL  = "gpt-5.6-luna"
const STEPS_MODEL    = "minimaxai/minimax-m3"
const THINKING_MODEL = "gpt-5.6-luna"
...
export function getModelProvider(model) {
  if (SEEKAI_MODELS.includes(model)) return "seekai"
  if (HCNSEC_MODELS.includes(model)) return "hcnsec"
  return "nvidia"
}
...
async function callSeekai(system, messages, model, timeoutMs = 60000) {
  console.log("[callSeekai] model:", model, "messages:", messages.length)
  ...
  try {
    const res = await fetch(SEEKAI_BASE, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "authorization": `Bearer ${SEEKAI_KEY}`,
        "accept": "application/json",
      },
      body: JSON.stringify(body),
      signal: controller.signal,
    ...
    if (!res.ok) {
      ...
      const isRateLimit = res.status === 429
      const isAuthError = res.status === 401 || res.status === 403
      console.error(`[callSeekai] model ${model} failed (${res.status}):`, err)
      const e = new Error(`SeekAI returned ${res.status}: ${err}`)
      e.isRateLimit = isRateLimit
      e.isAuthError = isAuthError
      e.status = res.status
      throw e
    ...
    console.log("[callSeekai] succeeded — output length:", output.length, "reasoning length:", reasoning.length)
    return { output, reasoning }
  } catch (e) {
    if (e.name === "AbortError") {
      const timeoutErr     = new Error(`SeekAI request to ${model} timed out after ${timeoutMs}ms`)
      timeoutErr.isTimeout = true
      throw timeoutErr
    }
    throw e
...
async function callHcnsec(system, messages, model, timeoutMs = 60000) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const isRateLimit = res.status === 429
      const isAuthError = res.status === 401 || res.status === 403
      console.error(`[callHcnsec] model ${model} failed (${res.status}):`, err)
      const e = new Error(`hcnsec.cn returned ${res.status}: ${err}`)
      e.isRateLimit = isRateLimit
      e.isAuthError = isAuthError
      e.status = res.status
      throw e
    ...
    if (e.name === "AbortError") {
      const timeoutErr     = new Error(`hcnsec.cn request to ${model} timed out after ${timeoutMs}ms`)
      timeoutErr.isTimeout = true
      throw timeoutErr
    }
    throw e
...
async function callWithFallback(system, messages, timeoutMs = 60000, preferredModel) {
  ...
  for (const model of modelOrder) {
    ...
    if (provider === "nvidia" && !NVIDIA_KEY) {
      console.warn(`[callWithFallback] skipping ${model} — no NVIDIA key`)
      continue
    }

    try {
      console.log(`[callWithFallback] trying "${model}" (${provider})`)
      let result
      if (provider === "seekai") {
        result = await callSeekai(system, messages, model, timeoutMs)
      } else if (provider === "hcnsec") {
        result = await callHcnsec(system, messages, model, timeoutMs)
      } else {
        result = await callNvidia(system, messages, model, timeoutMs)
      }
      if (result.output) {
        if (model !== preferred) {
...
export function normaliseResponse(result, model, reasoning = "", fellBack = false) {
  const provider = getModelProvider(model)
  const providerLabel = provider === "seekai" ? "SeekAI" : provider === "hcnsec" ? "hcnsec.cn" : "NVIDIA"
  result.__model         = model
  result.__provider      = providerLabel
...
export async function runGenerate({
  ...
  try {
    ...
    return {
      ...
      __provider:        getModelProvider(requestedModel) === "seekai" ? "SeekAI" : getModelProvider(requestedModel) === "hcnsec" ? "hcnsec.cn" : "NVIDIA",
...
export async function runFetchSteps(prompt) {
  ...
  try {
    const { output } = await callWithFallback(system, messages, 30000, STEPS_MODEL)
...
export async function runThinking({
  ...
  try {
    ...
    return {
      ...
      __model:         usedModel,
      __provider:      getModelProvider(usedModel) === "seekai" ? "SeekAI" : getModelProvider(usedModel) === "hcnsec" ? "hcnsec.cn" : "NVIDIA",
