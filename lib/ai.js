...
export const ALL_MODELS = [...NVIDIA_MODELS]
...
async function callWithFallback(system, messages, timeoutMs = 60000, preferredModel) {
  ...
  for (const model of modelOrder) {
    try {
      console.log(`[callWithFallback] trying "${model}"`)
      ...
      if (output) {
        ...
        return { output, reasoning, model, fellBack: model !== preferred }
      }
  ...
  throw new Error(`All NVIDIA models failed.${lastError ? " Last error: " + lastError.message : ""}`)
}
