...
const KIMI_BASE    = "https://api.hcnsec.cn/v1/chat/completions"

const NVIDIA_KEY = process.env.NVIDIA_API_KEY || ""
const KIMI_KEY   = "sk-I5e5KLNBS5kXwabGF3y92OrUEF2Tecxkcm1Jse0SY25YE6S5"
...
export const ALL_MODELS = [...KIMI_MODELS, ...NVIDIA_MODELS]

const DEFAULT_MODEL  = "DeepSeek-V4-Pro"
const STEPS_MODEL    = "minimaxai/minimax-m3"
const THINKING_MODEL = "DeepSeek-V4-Pro"
...
export function getModelProvider(model) {
  if (KIMI_MODELS.includes(model)) return "kimi"
  return "nvidia"
}
...
async function callKimi(system, messages, model, timeoutMs = 60000) {
  console.log("[callKimi] model:", model, "messages:", messages.length)
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const isRateLimit = res.status === 429
      const isAuthError = res.status === 401 || res.status === 403
      console.error(`[callKimi] model ${model} failed (${res.status}):`, err)
      const e = new Error(`Kimi returned ${res.status}: ${err}`)
      e.isRateLimit = isRateLimit
      e.isAuthError = isAuthError
      e.status = res.status
      throw e
    ...
    console.log("[callKimi] succeeded — output length:", output.length, "reasoning length:", reasoning.length)
    return { output, reasoning }
  } catch (e) {
    if (e.name === "AbortError") {
      const timeoutErr     = new Error(`Kimi request to ${model} timed out after ${timeoutMs}ms`)
      timeoutErr.isTimeout = true
      throw timeoutErr
    }
    throw e
...
async function callWithFallback(system, messages, timeoutMs = 60000, preferredModel) {
  ...
  for (const model of modelOrder) {
    ...
    try {
      ...
      let result
      if (provider === "kimi") {
        result = await callKimi(system, messages, model, timeoutMs)
      } else {
...
export function normaliseResponse(result, model, reasoning = "", fellBack = false) {
  ...
  result.__provider      = provider === "kimi" ? "Kimi" : "NVIDIA"
...
export async function runGenerate({
  ...
  try {
    ...
    return {
      ...
      __provider:        getModelProvider(requestedModel) === "kimi" ? "Kimi" : "NVIDIA",
...
export async function runThinking({
  ...
  try {
    ...
    return {
      ...
      __model:         usedModel,
      __provider:      getModelProvider(usedModel) === "kimi" ? "Kimi" : "NVIDIA",
