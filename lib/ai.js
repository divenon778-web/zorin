...
const STEPS_MODEL    = "DeepSeek-V4-Pro"
...
async function callProvider(baseUrl, apiKey, system, messages, model, timeoutMs, extraBody = {}) {
...
async function callWithFallback(system, messages, timeoutMs = 25000, preferredModel) {
  ...
  for (const model of modelOrder) {
    ...
    try {
      ...
      let result
      if (provider === "seekai") {
        result = await callProvider(SEEKAI_BASE, SEEKAI_KEY, system, messages, model, timeoutMs)
      } else if (provider === "hcnsec") {
        const extra = model.includes("nemotron") ? { extra_body: { chat_template_kwargs: { enable_thinking: true }, reasoning_budget: 8192 } } : {}
        result = await callProvider(HCNSEC_BASE, HCNSEC_KEY, system, messages, model, timeoutMs, extra)
      } else {
        const extra = model.includes("nemotron") ? { extra_body: { chat_template_kwargs: { enable_thinking: true }, reasoning_budget: 8192 } } : {}
        result = await callProvider(NVIDIA_BASE, NVIDIA_KEY, system, messages, model, timeoutMs, extra)
      }
      if (result.output) {
        if (model !== preferred) {
...
export async function callNvidiaWithFallback(system, messages, timeoutMs = 25000, preferredModel) {
...
export async function runGenerate({
  ...
  try {
    const { output, reasoning, model: pickedModel, fellBack: didFallBack } =
      await callWithFallback(sys, messages, 25000, requestedModel)
    raw           = output
...
export async function runFetchSteps(prompt) {
  ...
  try {
    const { output } = await callWithFallback(system, messages, 20000, STEPS_MODEL)
