...
const STEPS_MODEL    = "openai/gpt-oss-20b"
const THINKING_MODEL = "openai/gpt-oss-120b"

const MAX_TOKENS = 3072

const MAX_CONVERSATION_CHARS = 48000
...
async function callGroq(system, messages, model = DEFAULT_MODEL, apiKey, timeoutMs = 30000) {
  if (!apiKey) {
...
export async function callGroqWithFallback(system, messages, timeoutMs = 30000, preferredModel) {
  if (GROQ_KEYS.length === 0) {
    throw new Error(
      "No Groq API key configured. Set GROQ_API_KEY (or GROQ_API_KEY_1 through GROQ_API_KEY_9) " +
  ...
  for (const model of modelOrder) {
    for (const keyIndex of getGroqKeyAttemptOrder()) {
      ...
      try {
        console.log(`[callGroqWithFallback] trying model "${model}" with key ${keyIndex + 1}/${GROQ_KEYS.length}`)
        const { output, reasoning } = await callGroq(system, messages, model, apiKey, timeoutMs)
        if (output) {
          advanceGroqKeyIndex(keyIndex)
          if (model !== preferredModel) {
            console.warn(`[callGroqWithFallback] fell back from "${preferredModel}" to "${model}"`)
          }
          return { output, reasoning, model, fellBack: model !== preferredModel }
        }
        console.warn(`[callGroqWithFallback] model ${model} key ${keyIndex + 1} returned empty, trying next...`)
        advanceGroqKeyIndex(keyIndex)
      } catch (e) {
        console.warn(`[callGroqWithFallback] model ${model} key ${keyIndex + 1} failed:`, e.message)
        advanceGroqKeyIndex(keyIndex)
        lastError = e
        continue
      }
  ...
  throw new Error(`All Groq models/keys failed.${lastError ? " Last error: " + lastError.message : ""}`)
}
...
export async function runGenerate({
  ...
  try {
    const { output, reasoning, model: pickedModel, fellBack: didFallBack } =
      await callGroqWithFallback(sys, messages, 30000, requestedModel)
    raw           = output
    usedReasoning = reasoning || ""
    usedModel     = pickedModel
    fellBack      = didFallBack
  } catch (e) {
    console.error("[runGenerate] all Groq models/keys failed:", e)
    return {
      type:              "chat",
...
export async function runFetchSteps(prompt) {
  ...
  try {
    const { output } = await callGroqWithFallback(system, messages, 20000, STEPS_MODEL)
    ...
    console.error("[runFetchSteps] all Groq models/keys failed:", e)
  }
...
export async function runThinking({
  ...
  try {
    ...
    console.error("[runThinking] all Groq models/keys failed:", e)
    return {
      type:            "thinking",
      thinking_steps:  [],
