...
const STEPS_MODEL    = "minimax-m3"
const THINKING_MODEL = "minimax-m3"
...
export async function runGenerateStream({
  ...
  for (const m of modelOrder) {
    try {
      const { stream, provider } = await callHcnsecStream(sys, messages, m, 55000)
      if (!stream) continue
...
export async function runGenerate({
  ...
  try {
    const { output, reasoning, model: pickedModel, fellBack: didFallBack, provider: pickedProvider } =
      await callWithFallback(sys, messages, 55000, requestedModel)
    raw           = output
