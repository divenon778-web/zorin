...
const SEEKAI_BASE = "https://seekai.cc/v1/messages"
const SEEKAI_KEY  = "sk-biIVw8mvqkAYFbmCB1lfvoL8vATyk4Ivirjfq7AnAVinmeKj"
const SEEKAI_MODEL = "claude-opus-4-8"
...
const STEPS_MODEL    = "claude-opus-4-8"
const THINKING_MODEL = "claude-opus-4-8"
...
function getProviderConfig(model) {
  return { base: SEEKAI_BASE, key: SEEKAI_KEY, provider: "seekai", model: SEEKAI_MODEL }
}
...
function createThinkingStripTransform(encoder, metaLine, onFirstChunk) {
  ...
  return new TransformStream({
    transform(chunk, controller) {
      ...
      const raw = anthropicBuffer + new TextDecoder().decode(chunk)
        ...
        if (line.startsWith("data: ")) {
          ...
          try {
            ...
            if (parsed.type === "content_block_delta" && parsed.delta?.text) {
              const cleaned = processText(parsed.delta.text)
    ...
    flush(controller) {
      if (anthropicBuffer) {
        if (anthropicBuffer.startsWith("data: ")) {
          const data = anthropicBuffer.slice(6).trim()
          if (data !== "[DONE]") {
            try {
              const parsed = JSON.parse(data)
              if (parsed.type === "content_block_delta" && parsed.delta?.text) {
