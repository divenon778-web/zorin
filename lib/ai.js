...
const OLLAMA_BASE = "https://api.orcarouter.ai/v1/chat/completions"
const OLLAMA_MODEL = "deepseek/deepseek-v4-pro-free"
const OLLAMA_API_KEY = process.env.ORCAROUTER_API_KEY || ""

export const ALL_MODELS = [
  "deepseek/deepseek-v4-pro-free",
]

const DEFAULT_MODEL  = "deepseek/deepseek-v4-pro-free"
const STEPS_MODEL    = "deepseek/deepseek-v4-pro-free"
const THINKING_MODEL = "deepseek/deepseek-v4-pro-free"
    ...
    transform(chunk, controller) {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (content) {
            chunkCount++
            if (chunkCount <= 3) console.log(`[stream chunk ${chunkCount}]`, content.slice(0, 100))
          ...
          if (chunkCount === 0) console.log("[stream unparseable]", jsonStr.slice(0, 200))
        }
      }
    },
...
export function callHcnsecStream(system, messages, model, timeoutMs = 120000) {
  ...
  return p.then(res => {
      ...
      async start(controller) {
        try {
          while (true) {
            ...
            if (done) break
            if (rawSample.length < 500) rawSample += decoder.decode(value, { stream: true })
            controller.enqueue(value)
          }
          console.log(`[callHcnsecStream] raw sample: ${rawSample.slice(0, 500)}`)
          controller.close()
        } catch (e) {
          controller.error(e)
        }
    ...
    return { stream: passthrough, model, provider }
  }).catch(e => {
    clearTimeout(timer)
