...
const VYCEAI_BASE = "https://vyceai.com/v1/chat/completions"
const VYCEAI_MODEL = "gpt-5.6-luna"
const VYCEAI_API_KEY = process.env.VYCEAI_API_KEY || process.env.NEW_API_KEY || process.env.SEEKAI_API_KEY || process.env.TABITOKEN_API_KEY || process.env.TOKENROUTER_API_KEY || process.env.ORCAROUTER_API_KEY || ""

// Keep legacy names for backwards compat — all point to VyceAI now
const SEEKAI_BASE = VYCEAI_BASE
const SEEKAI_MODEL = VYCEAI_MODEL
const SEEKAI_API_KEY = VYCEAI_API_KEY

const TABI_BASE = VYCEAI_BASE
const TABI_MODEL = VYCEAI_MODEL
const TABI_API_KEY = VYCEAI_API_KEY

const OLLAMA_BASE = VYCEAI_BASE
const OLLAMA_MODEL = VYCEAI_MODEL
const OLLAMA_API_KEY = VYCEAI_API_KEY

export const ALL_MODELS = [
  "gpt-5.6-luna",
  // legacy aliases — still accepted (mapped to vyceai)
  "gpt-5.6-sol",
...
const STEPS_MODEL    = "gpt-5.6-luna"
...
function getProviderConfig(model) {
  ...
  if (model && model.startsWith("qwen/")) {
    ...
    const tokenRouterKey = process.env.TOKENROUTER_API_KEY || process.env.VYCEAI_API_KEY || process.env.NEW_API_KEY || ""
    return { base: tokenRouterBase, provider: "tokenrouter", model, apiKey: tokenRouterKey }
  }
  if (model && model.startsWith("claude-")) {
    return { base: "https://tabitoken.com/v1/chat/completions", provider: "tabitoken", model, apiKey: process.env.TABITOKEN_API_KEY || process.env.VYCEAI_API_KEY || process.env.NEW_API_KEY || TABI_API_KEY }
  }
  // vyceai primary — all gpt-5.6-* map to gpt-5.6-luna
  return { base: VYCEAI_BASE, provider: "vyceai", model: VYCEAI_MODEL, apiKey: VYCEAI_API_KEY }
}
...
export function callHcnsecStream(system, messages, model, timeoutMs = 120000) {
  ...
  if (!apiKey) {
    const e = new Error("Missing VYCEAI_API_KEY env var (fallback NEW_API_KEY also missing)")
    e.isAuthError = true
    e.status = 500
...
async function callHcnsec(system, messages, model, timeoutMs = 120000) {
  ...
  if (!apiKey) throw Object.assign(new Error("Missing VYCEAI_API_KEY env var (fallback NEW_API_KEY also missing)"), { isAuthError: true, status: 500 })
...
export function normaliseResponse(result, model, reasoning = "", fellBack = false, provider = "vyceai") {
