...
const STEPS_MODEL    = "nvidia/nemotron-3-ultra-550b-a55b"
...
function getProviderConfig(model) {
  if (model.startsWith("nvidia/")) {
    return { base: NVIDIA_BASE, key: NVIDIA_KEY, provider: "nvidia" }
  }
  if (model === "deepseek-v4-flash") {
    return { base: BAI_BASE, key: BAI_KEY, provider: "api.b.ai" }
  }
  if (model === "deepseek-v4-pro-free") {
    return { base: BYNARA_BASE, key: BYNARA_KEY, provider: "bynara.id" }
  }
  return { base: HCNSEC_BASE, key: HCNSEC_KEY, provider: "hcnsec.cn" }
}

export function callHcnsecStream(system, messages, model, timeoutMs = 60000) {
  const { base, key, provider } = getProviderConfig(model)
  console.log(`[callHcnsecStream] model: ${model} (${provider}), messages: ${messages.length}`)
...
async function callHcnsec(system, messages, model, timeoutMs = 60000) {
  const { base, key, provider } = getProviderConfig(model)
  console.log(`[callHcnsec] model: ${model} (${provider}), messages: ${messages.length}`)
...
export async function runGenerate({
  ...
  try {
    const { output, reasoning, model: pickedModel } =
      await callHcnsec(sys, messages, requestedModel, 10000)
    raw           = output
    usedReasoning = reasoning || ""
    usedModel     = pickedModel
    fellBack      = false
    provider      = getProviderConfig(pickedModel).provider
  } catch (e) {
