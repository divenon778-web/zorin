...
async function callHcnsecStream(system, messages, model, timeoutMs = 60000) {
  console.log("[callHcnsecStream] model:", model, "messages:", messages.length)
...
export async function runGenerate({
  ...
  try {
    const { output, reasoning, model: pickedModel, fellBack: didFallBack } =
      await callWithFallback(sys, messages, 30000, requestedModel)
    raw           = output
...
export async function runGenerateStream({
  ...
  console.log("[runGenerateStream] start — prompt:", prompt.slice(0, 100))
  ...
  for (const m of modelOrder) {
    const isDeepSeek = m === "DeepSeek-V4-Pro"
    const modelTimeout = isDeepSeek ? 55000 : 30000

    try {
      console.log(`[runGenerateStream] trying "${m}"`)
      const stream = await callHcnsecStream(sys, messages, m, modelTimeout)
      if (stream) {
        ...
        const modelUsed = m
        const metaChunk = encoder.encode(JSON.stringify({ __model: modelUsed, __provider: "hcnsec.cn" }) + "\n")
        ...
        const combined = metaStream.pipeThrough(transform)
